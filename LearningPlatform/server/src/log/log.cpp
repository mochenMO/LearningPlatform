...
LogEventManager* mochen::log::getDefaultLogEventManager() // 注意在C++11及以上版本标准中，规定静态局部变量初始化的线程安全性
{
...
// 全局函数
Logger* mochen::log::getDefaultLogger()
{
...
void mochen::log::logFormatFuntion_detailed(std::stringstream& _ss, LogEvent& _logEvent)
{
...
void mochen::log::logFormatFuntion_briefly(std::stringstream& _ss, LogEvent& _logEvent)
{
	char timeString[24] = { 0 };
	struct tm* pt = nullptr;     // 注意 pt 指向的内存空间由系统管理，无需手动释放
...
FileLogAppender::FileLogAppender(const std::string& _filename, int _maxSize)
{
	...
	const char* filename = _filename.c_str() + _filename.size();
	while (filename != _filename.c_str()) {
	...
	m_filename = filename;
	m_path = _filename.substr(0, filename - _filename.c_str());
	
	std::string temp = _filename;
	temp += ".log";
	if ((m_fp = fopen(temp.c_str(), "a")) == nullptr) {
		throw std::logic_error("FileLogAppender::FileLogAppender(): open file failded");    // std::logic_error异常对象中会自动包含文件名和行号等调试信息
	}
}
...
void FileLogAppender::open(const std::string& _filename)
{
	...
	if ((m_fp = fopen(temp.c_str(), "a")) == nullptr) {
		throw std::logic_error("FileLogAppender::open(): open file failded");    // std::logic_error异常对象中会自动包含文件名和行号等调试信息
	}
}
...
int FileLogAppender::getFileSize()
{
	int size = 0;
...
void FileLogAppender::setFileSize(int _size)
{
	m_maxSize = _size;
}
...
bool LogEventManager::isFindLogger(const std::string& _loggername)
{
	return (m_LogAppenderListMap->find(_loggername) != m_LogAppenderListMap->end());
}


std::map<std::string, std::list<std::shared_ptr<LogAppender>>>* LogEventManager::getLogAppenderListMap()
{
...
void LogEventManager::dealLogEvent_threadFuntion()
{
	...
	while (m_isCanExit == false || m_ptrRead->m_next != nullptr)  // 当 m_ptrRead = m_ptrWrite 时 m_ptrRead->m_next 也会受到内存屏障保护
	{
		if (m_ptrRead->m_next != nullptr)
		{
			...
			if (m_logEventQueue-> m_next != m_ptrRead) {   // 头删法。当 m_logEventQueue = m_ptrWrite 时 m_logEventQueue->m_next 也会受到内存屏障保护（注意该删除方式最后 m_ptrRead=m_ptrWrite 只存在一个哨兵节点和一个元素节点共两个节点）
