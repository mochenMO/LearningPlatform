...
extern void logFormatFuntion_detailed(std::stringstream& _ss, LogEvent& _logEvent);

// [时:分:秒][日志等级][简要的文件名]:[行号][日志信息]
extern void logFormatFuntion_briefly(std::stringstream& _ss, LogEvent& _logEvent);
...
private:
	...
	FILE* m_fp;
	int m_maxSize;
#define M_FILEMAXSIZE 1048576    // 10MB
public:
	FileLogAppender();
	...
	// std::string setFilename();      // 为了m_filename和m_fp相匹配，禁止用户直接改文件名，而是通过open去改文件名
	std::string getFilename();

	int getFileSize();
	void setFileSize(int _size);
...
public:
	...
	bool isFindLogger(const std::string& _loggername);
	std::map<std::string, std::list<std::shared_ptr<LogAppender>>>* getLogAppenderListMap();
...
extern LogEventManager* getDefaultLogEventManager();
...
public:
	...
	// void setLoggerName(const std::string _loggerName);    // 禁止用户改 loggerName
...
// 声明全局函数，代替在头文件中声明全局变量，且返回的指针类型，避免调用拷贝或移动相关函数
extern Logger* getDefaultLogger();
