...
log::Logger* createJsonLogger()   // 初始化 JsonLogger 的函数 (注意该函数只能被运行一次)
{
	...
	//std::shared_ptr<log::FileLogAppender> temp = std::make_shared<log::FileLogAppender>();
	//temp->open("json");
	//JsonLogger.addAppender(temp);

	static log::Logger JsonLogger("JsonLogger", log::LogLevel::debug, std::make_shared<log::FileLogAppender>("server/logs/json"));

	return &JsonLogger;
}

log::Logger* mochen::json::getJsonLogger()  // 获得JsonLogger的函数，写该函数是为了为多文件共享该JsonLogger做准备（注意加命名空间）
{
...
bool JsonParser::open(const std::string& _filename)
{
	...
	if ((fp = fopen(_filename.c_str(), "r")) == nullptr) {
		logger_error(JsonLogger, "Json::open(): open file failed <%s>", _filename.c_str());
		return false;
	}
